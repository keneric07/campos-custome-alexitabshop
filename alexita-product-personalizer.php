...
final class Alexita_Product_Personalizer {
	...
	private static function get_countries_for_js() {
		...
		foreach ( self::get_countries_list() as $country ) {
			...
			$list[] = array(
				...
				'featured' => ! empty( $country['featured'] ),
			);
		}
		...
		wp_localize_script(
			...
			array(
				...
				'labels'          => array(
					...
					'worldcupCheck'   => __( 'El país clasificó al Mundial 2026', 'alexita-product-personalizer' ),
					...
					'changePhoto'     => __( 'Cambiar foto', 'alexita-product-personalizer' ),
					'photoTapUpload'  => __( 'Toca para subir', 'alexita-product-personalizer' ),
	...
	public static function validate_and_prepare_personalization( $passed, $product_id, $quantity, $variation_id = 0, $variations = array(), $cart_item_data = array() ) {
		...
		for ( $i = 0; $i < $quantity; $i++ ) {
			...
			$country       = null;
			$use_country_list = ! empty( $players_post[ $i ]['worldcup_list'] );
